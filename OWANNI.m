...
% version: OWANNI, similar to FRNNI

function[Data] = OWANNI(Data, Flag, K)
...
for itemp = 1:size(cmis,1)
    ...
    for jtemp = 1:size(tempMisColIndex,1)  
        ...
        for i = 1:K
            ...
            lower_approx(i,:) = owaWeight*max(1-R_yt, R_tz);
            upper_approx(i,:) = (2/(K-1)-owaWeight)*min(R_yt, R_tz);
